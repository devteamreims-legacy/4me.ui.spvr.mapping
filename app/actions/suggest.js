...
export const MAPPING_FETCH_SUGGESTIONS_START = 'MAPPING_FETCH_SUGGESTIONS_START';
export const MAPPING_FETCH_SUGGESTIONS_FAIL = 'MAPPING_FETCH_SUGGESTIONS_FAIL';
export const MAPPING_FETCH_SUGGESTIONS_COMPLETE = 'MAPPING_FETCH_SUGGESTIONS_COMPLETE';


export function fetchSuggestions(cwpId) {
  return (dispatch, getState) => {
    // Check if loading
    let isLoading = isSuggestionLoading(getState());
    ...
    */
    ...
    const apiUrl = api.rootPath + api.mapping.suggest(cwpId);
...
export function fail(cwpId, rawError) {
  ...
  return {
    type: MAPPING_FETCH_SUGGESTIONS_FAIL,
    cwpId,
    error
  };
}
...
export function start(cwpId) {
  return {
    type: MAPPING_FETCH_SUGGESTIONS_START,
    cwpId
  };
}
...
export function complete(cwpId, suggestions = []) {
  return {
    type: MAPPING_FETCH_SUGGESTIONS_COMPLETE,
    cwpId,
    suggestions
  };
}